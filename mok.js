...
function watchConfig(file) {
	...
	fs.watch(file, function (eventType) {
		if (update && eventType==='change') { //防止重复触发
			...
			update = false;
			setTimeout(function(){update = true}, 200);
		}
	});
}
	...
	for (i = 0; i < len; i++) {
		...
		if (match) { //console.log(match);
			if (route.project) {
				...
				} else if (test_min[route.project]) {
...
function execCmd(req_path, request, response) {
	...
	if (!prj || !prj_conf) {
		response.end(global.HEAD_HTML.replace('{{title}}', '无效的项目名')+
			'无效的项目名：'+prj+
			'。请在配置文件里检查是否有该项目的配置。</body></html>');
		return;
	}
	
	var cmd = argv._cmd;
	if (cmd==='b' || cmd==='build') { //打包压缩
		prj_conf.compress_cmd = CONF.compress_cmd;
		if (prj_conf.comb_mode) {
			require('./mok4js/build_comb').build(argv, prj_conf, response);
			return;
		}
		...
		require(type?'./moktext/'+type:'./mok4js/build').build(argv, prj_conf, response);
	
	} else if (cmd==='min') { //切换压缩文件模式
		if (prj_conf.type === 'css') {
			require('./moktext/css').test_min(prj_conf, response);
			return;
		}
		test_min[prj] = !!prj_conf.__hasbuilt && !test_min[prj];
		response.write(global.HEAD_HTML.replace('{{title}}', '切换测试JS压缩文件模式'));
		if (prj_conf.__hasbuilt) {
			response.end('已 <b>'+(test_min[prj] ? '<em>切换到</em>' : '取消')+
				'</b> 测试JS压缩文件模式。</body></html>')
		} else {
			response.end('未构建项目，<b>不能切换到</b> 测试JS压缩文件模式。<br/>'+
				'另：修改配置文件后需要重新构建项目。</body></html>');
		}
		prj_conf.comb_mode && test_min[prj] &&
...
//创建服务器
function createServer(routes, default_port) {
	...
	if (CONF.https_routes) {
		...
		require('https').createServer({
			key: fs.readFileSync(__dirname+'/common/ssl/privatekey.pem'),
			cert: fs.readFileSync(__dirname+'/common/ssl/certificate.pem')
